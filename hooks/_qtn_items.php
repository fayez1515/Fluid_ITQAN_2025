<?php
	...
	/**
	 * @file
	 * This file contains hook functions that get called when data operations are performed on '_qtn_items' table. 
	...
	*/
	...
	function _qtn_items_init(&$options, $memberInfo, &$args) {
	...
	*/
	...
	function _qtn_items_header($contentType, $memberInfo, &$args) {
		$header='';
	...
	*/
	...
	function _qtn_items_footer($contentType, $memberInfo, &$args) {
		$footer='';
	 ...
	 *     $data['item'], $data['note'], $data['suggestedprice'], $data['qty'], $data['value'], $data['vat'], $data['item_arabic'], $data['code'], $data['partno']
	...
	*/
	...
	function _qtn_items_before_insert(&$data, $memberInfo, &$args) {
	

$get_my_valuea = $data['id_projects']  ;	

//$data['test1'] = $get_my_valuea;

$get_my_value1 = sqlValue("SELECT `sign_count` from `projects`  where `projects`.`id` = '{$get_my_valuea}' ");
...
if( $get_my_value1 > 0 ) return FALSE;
	 ...
	 * For this table, the array items are: 
	 *     $data['item'], $data['note'], $data['suggestedprice'], $data['qty'], $data['value'], $data['vat'], $data['item_arabic'], $data['code'], $data['partno'], $data['total'], $data['id_qtn']
	...
	*/
	...
	function _qtn_items_after_insert($data, $memberInfo, &$args) {
	...
	*/
	...
	function _qtn_items_before_update(&$data, $memberInfo, &$args) {
		
 


$get_my_value2 = sqlValue("SELECT `sign_count_projects` from `_qtn_items` where `id_items`='{$data['selectedID']}'");
if( $get_my_value2 > 0 ) return FALSE;
	 ...
	 * An associative array where the keys are field names and the values are the field data values.
	 * For this table, the array items are: 
	 *     $data['item'], $data['note'], $data['suggestedprice'], $data['qty'], $data['value'], $data['vat'], $data['item_arabic'], $data['code'], $data['partno'], $data['total'], $data['id_qtn'], $data['id_qnt_items']
	...
	*/
	...
	function _qtn_items_after_update($data, $memberInfo, &$args) {
	...
	*/
	...
	function _qtn_items_before_delete($selectedID, &$skipChecks, $memberInfo, &$args) {
	...
	*/
	...
	function _qtn_items_after_delete($selectedID, $memberInfo, &$args) {
	...
	*/
	...
	function _qtn_items_dv($selectedID, $memberInfo, &$html, &$args) {
	...
	*/
	...
	function _qtn_items_csv($query, $memberInfo, &$args) {
	...
	*/
	...
	function _qtn_items_batch_actions(&$args) {
